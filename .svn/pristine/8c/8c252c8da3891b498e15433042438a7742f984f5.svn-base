...
Ext.define('Ext.app.PortalDropZone', {
    ...
    createEvent: function(dd, e, data, col, c, pos) {
    	this.columnTs = data.panel.ownerCt;
    	if(this.cloflag == undefined) {
    		this.c = c;
    		this.pos = pos;
    		this.cloflag = "";
    		this.stColPos = pos;
    ...
    notifyOver: function(dd, e, data) {
        ...
        for (len; pos < len; pos++) {
            ...
            if (h === 0) {
                overSelf = true;
            } else if ((overPortlet.el.getY() + h) > xy[1]) {
                match = true;
                break;
            }
        }
...
//            }
            ...
            this.lastPos = {
                c: overColumn,
                col: colIndex,
                p: overSelf || (match && overPortlet) ? pos : (pos-1)
            };
    ...
    notifyDrop: function(dd, e, data) {
        ...
        if (this.portal.fireEvent('validatedrop', dropEvent) !== false && 
            ...
            var rempanel;
            if (pos !== false) {
				//resultMap存储变动座位的考试信息
				//key为考生id，value为"列号@行号"
            	
            	//考生id不为空，也就是panel不是空座位
            	if(c.items.items[pos].id != 'empty') {
					resultMap.replace(c.items.items[pos].id, this.columnTs.id + "@" + (this.stColPos+1));				
            	}
            	
            	var thisx = this.c.items.items[this.pos].y;
            	var thisy = this.c.items.items[this.pos].x;
            	this.c.items.items[this.pos].y = c.items.items[pos].y;
            	this.c.items.items[this.pos].x = c.items.items[pos].x;
            	c.items.items[pos].x = thisx;
            	c.items.items[pos].y = thisy;
            	var xy = document.getElementById(c.items.items[pos].meapid).value;
            	document.getElementById(c.items.items[pos].meapid).value = document.getElementById(this.c.items.items[this.pos].meapid).value;
            	document.getElementById(this.c.items.items[this.pos].meapid).value = xy;
            	if(panel.id != 'empty') {
					resultMap.replace(panel.id, c.id + "@" + (pos+1));				
            	}
            	console.info(resultMap);
            	
            	this.columnTs.insert(this.stColPos,c.items.items[pos]);
            	this.cloflag = undefined;
                c.insert(pos, panel);
            } else {
                c.add(panel);
            }
