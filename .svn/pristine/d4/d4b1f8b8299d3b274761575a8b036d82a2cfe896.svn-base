...
@Controller
@Scope("prototype")
public class ExamTeacherArrangeIAction extends BaseAction implements ModelDriven<CusKwMonitorteacher>{
	...
	public void getListPage() {
		String jklssl = getRequest().getParameter("jklssl");
		String sfcxap = getRequest().getParameter("sfcxap");
		String nj = getRequest().getParameter("nj");
		String lcid = getRequest().getParameter("lcid");
		String kdid = getRequest().getParameter("kdid");
		try {
			writerPrint(examTeacherArrangeService.getInvigilatorArrangeTeaListPage(jklssl,sfcxap,nj,lcid,kdid).getResultList());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
