...
<body style="font-size:12px">
	...
	if(list!=null && list.size()>0){
		for(int i=0;i<list.size();i++){
			...
			if(!km.equals(map.get("EXAMNAME"))){
				km=map.get("EXAMNAME").toString();%>
				<div><div style="height: 26px;text-align:left;padding-top: 10px"><label>考试名称：<%=km %></label></div></div>
			...
			if(i+1<list.size()){
				...
				if(!km.equals(map2.get("EXAMNAME"))){%>
					<tbody>
				</table>
