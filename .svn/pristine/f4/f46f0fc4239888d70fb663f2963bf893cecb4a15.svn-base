Ext.extend(system.application.baseClass,{
    ...
    initComponent :function(){
		...
		var cm = [
			...
			{header: "组考单位数量",   align:"center", sortable:true, dataIndex:"CKXXSL"}
		];
		this.grid = new Ext.ux.GridPanel({
			...
			title:"组考单位安排",
			tbar:[ 
			      "->",{xtype:"button",id:"examSchool",text:"组考单位安排",iconCls:"p-icons-update",handler:this.examSchoolArrange,scope:this}			  
			     ],
			page:true,
			rowNumber:true,
			region:"center",
			action:"examroomarrange_getListPage.do",
			fields :["LCID","EXAMNAME","STARTWEEK","STARTDAY","STARTDATE","ENDWEEK","ENDDAY","ENDDATE","EXAMTYPEM","XNMC","XQMC","COUNTKD","COUNTKS","CKXXSL"],
			border:false
		});
		//搜索区域
		var xn_find = new Ext.ux.form.XnxqField({width:180,id:"esxn_find",readOnly:true});
		var cx = new Ext.Button({x:17,y:-10,cls:"base_btn",text:"查询",handler:this.selectExamSchoolAp,scope:this});
    	...
    	this.panel = new Ext.Panel({layout:"border",region:"center",items:[this.search,this.grid]});
		...
		var cm1 = [sm1,
			...
			{header: "组考单位地址",   align:"center", sortable:true, dataIndex:"XXDZ"},
			{header: "是否参加本次考试",   align:"center", sortable:true, dataIndex:"SFCK"}
		];
		this.gridArrange = new Ext.ux.GridPanel({
			cm:cm1,
			sm:sm1,
			title:"组考单位安排",
			tbar:["->",{xtype:"button",text:"返回",iconCls:"p-icons-unpassing",handler:this.returnArrange,scope:this},
			      "->",{xtype:"button",text:"取消组考",iconCls:"p-icons-delete",handler:this.deleteArrange,scope:this},
			      //"->",{xtype:"button",text:"手动安排",iconCls:"p-icons-update",handler:this.arrangeStudent,scope:this},
			      "->",{xtype:"button",text:"组考设置",iconCls:"p-icons-update",handler:this.seatingArrangements,scope:this}
			     ],
			page:true,
			rowNumber:true,
			region:"center",
		...
		var cx1 = new Ext.Button({x:17,y:-10,cls:"base_btn",text:"查询",handler:this.selectExamSchool,scope:this});
		var cz1 = new Ext.Button({x:87,y:-10,cls:"base_btn",text:"重置",handler:function(){this.searchArrange.getForm().reset()},scope:this});
		this.searchArrange = new Ext.form.FormPanel({
			region: "north",
			height:90,
			items:[{  
				layout:'form',  
				...
				items: [{
						...
						baseCls:"table",
						items:[
							{html:"组考单位：",baseCls:"label_right",width:170},
							{items:[organ_sel],baseCls:"component",width:210},
							{layout:"absolute",items:[cx1,cz1],baseCls:"component_btn",width:170}
						] 
                 	}]  
		     	}]  
	    });
		this.panelArrange = new Ext.Panel({layout:"border",region:"center",items:[this.searchArrange,this.gridArrange]});
		
    	this.tabPanel = new Ext.TabPanel({   
            activeTab: 0,
       		headerStyle: 'display:none',
       		border:false,
            items: [this.panel,this.panelArrange]   
        });
	},
	createExamSchoolEditForm:function(){
		...
		var cm = [
			...
			{header: "组考单位代码",   align:"center", sortable:true, dataIndex:"XXBSM"},
			{header: "组考单位名称",   align:"center", sortable:true, dataIndex:"XXMC"},
			{header: "组考单位剩余人数",   align:"center", sortable:true, dataIndex:"SYRS"},
    ...
    selectExamSchool:function(){
    	//查询考点人数安排情况
    	var organ_sel=this.searchArrange.getForm().findField('organ_sel').getCodes();
    	this.gridArrange.$load({"organ_sel":organ_sel,"lcid":this.lcid});
	},
	seatingArrangements:function(){
		//显示自动安排弹出框
		var selectedTeacher = this.gridArrange.getSelectionModel().getSelections();
		...
		if(xxcode==""){
			Ext.MessageBox.alert("消息","你选择的组考单位已参考，无需再次设置！");
			return;
	    }else{
	...
	arrangeStudent:function(){
		//显示手动安排考生弹出框
		var selectedSchool = this.gridArrange.getSelectionModel().getSelections();
	...
	deleteArrange:function(){
	    ...
	    if(xxcode==""){
	    	Ext.MessageBox.alert("消息","你选择的组考单位并不没有参加考试，无需取消！");
	    	return;
	    }else{
	    	xxcode=xxcode.substring(0,xxcode.length-1);
	    }
    ...
    selectExamSchoolAp:function(){
		var xn_find=Ext.getCmp('esxn_find').getCode();
		this.grid.$load({"xnxqId":xn_find});
	},
    examSchoolArrange:function(){
    	//跳转到组考单位安排页面
		var selectedSchool = this.grid.getSelectionModel().getSelections();
    	if(selectedSchool.length != 1){
    		Ext.MessageBox.alert("消息","请选择一条记录进行组考单位安排！");
    		return;
    	}
    	this.lcid = selectedSchool[0].get("LCID");
    	this.gridArrange.getStore().removeAll();
    	this.tabPanel.setActiveTab(this.panelArrange);    	    	
    },
