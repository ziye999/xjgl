...
import com.jiajie.service.signup.ManagementService;
...
public class ManagementServiceImpl extends ServiceBridge implements ManagementService{
	...
	public PageBean getList(String xn,String dwid,MBspInfo mbf) {
		...
		String 	sql = "select DISTINCT s1.*,(s1.SL-s1.YZP) MZP from (select DISTINCT rd.BMLCID,rd.SHSM,rd.XN,rd.XQM,rd.EXAMNAME,rd.STARTWEEK,rd.SL,rd.STARTDAY,"+
				"rd.STARTDATE,rd.ENDWEEK,rd.ENDDAY,rd.ENDDATE,rd.EXAMTYPEM,rd.DWID,rd.DWTYPE,xx.SSZGJYXZDM," +
				"(SELECT count(PATH) FROM zxxs_xs_pic z LEFT JOIN cus_kw_examinee e on z.XS_JBXX_ID=e.XS_JBXX_ID where e.bmlcid=rd.bmlcid and PATH!='') YZP, " +
				"case rd.SHFLAG when '1' then '审核通过' when '0' then '已上报' when '2' then '审核不通过' when '3' then '已缴费' else  '未上报' end as SHFLAG,xq.mc XQ,"+
				...
				"case rd.fpzt when '1' then '已申请' when '2' then '已审核' else '未申请' end as fpzt "+
...
//		String 	sql = "select DISTINCT s1.*,case (s1.SL-s1.YZP) when (s1.SL-s1.YZP>0) then (s1.SL-s1.YZP) else '0' end as MZP from (" +
//				"select DISTINCT rd.BMLCID,rd.SHSM,rd.XN,rd.XQM,rd.EXAMNAME,rd.STARTWEEK,IFNULL(rd.SL,'0') SL,rd.STARTDAY,"+
//				"rd.STARTDATE,rd.ENDWEEK,rd.ENDDAY,rd.ENDDATE,rd.EXAMTYPEM,rd.DWID,rd.DWTYPE,xx.SSZGJYXZDM," +
//				"(SELECT count(PATH) FROM zxxs_xs_pic z LEFT JOIN cus_kw_examinee e on z.XS_JBXX_ID=e.XS_JBXX_ID where e.bmlcid=rd.bmlcid and PATH!='') YZP, " +
//				"case rd.SHFLAG when '1' then '审核通过' when '0' then '已上报' when '2' then '审核不通过' when '3' then '已缴费' else  '未上报' end as SHFLAG,xq.mc XQ,"+
//				"IFNULL(jb.region_edu,(select (select region_edu from com_mems_organ where region_code=a.SSZGJYXZDM) from zxxs_xx_jbxx a where a.xx_jbxx_id=rd.dwid)) MC,"+
...
//				"' or xx.SSZGJYXZDM in (select REGION_CODE from com_mems_organ where PARENT_CODE='"+mbf.getOrgan().getOrganCode()+
//				"') or xx.SSZGJYXZDM in (select REGION_CODE from com_mems_organ where PARENT_CODE in (select REGION_CODE from com_mems_organ where PARENT_CODE='"+mbf.getOrgan().getOrganCode()+"')))) s1 where 1=1";
		if("4".equals(mbf.getUserType())){
			sql += " and  s1.dwid='"+mbf.getOrgan().getOrganCode()+"'";
		}
		if ("2".equals(mbf.getUserType())) {			
			sql += " and  s1.SSZGJYXZDM ='"+mbf.getOrgan().getOrganCode()+"'";
		}
		if(xn==null || "".equals(xn)){
...
//		System.out.println(mbf.getOrgan().getOrganName());
	...
	public com.jiajie.bean.MsgBean updateEaxminee(String bmlcid) {
		...
		try {
			...
			if("0".equals(ls.get(0).get("SHFLAG"))||"1".equals(ls.get(0).get("SHFLAG"))){
				...
				return getMsgBean(false,"该轮次已上报或已审核，参考单位不能进行报名操作！");
			}
			String sql = "update cus_kw_examinee set flag='0' where bmlcid='"+bmlcid+"'";
			update(sql);
			String sql2 = "update cus_kwbm_examround set shflag='0' where bmlcid='"+bmlcid+"'";
			update(sql2);
			return getMsgBean(true, "上报成功！下一步上传缴费凭证！");
			...
			return getMsgBean(false, "不是报名时间，不能上报！");
		}
	}
